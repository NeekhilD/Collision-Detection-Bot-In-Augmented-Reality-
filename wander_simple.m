...
function wander_simple(posn, rad, wdia, course, dt)
...
%% Drive Around
for i = 1:dt:1000
    rangeL = rangefinder([posn(1), posn(2), posn(3)+pi/6], rad, course); %left
    rangeR = rangefinder([posn(1), posn(2), posn(3)-pi/6], rad, course); %right
    rangeC = rangefinder(posn, rad,course); %center
    rangeLC = rangefinder([posn(1), posn(2), posn(3)+pi/12], rad, course); %left
    rangeRC = rangefinder([posn(1), posn(2), posn(3)-pi/12], rad, course); %left    
    ...
    if(detectcollision(posn,rad,course))
        disp(['Collision Detected at (' num2str(posn(1)) ',' num2str(posn(2)) ')']);
    end
